 ...
 *
 * This list includes both System and Unicode commands, as well as all the
 * various aircraft commands.
 *
 * Aliased commands map to a single root command that is shared among all
 * aliases. The values here then map to a `AIRCRAFT_COMMAND_DEFINITION` which
 * contains `validate` and `parse` functions for each root command. Some
 * commands have very unique demands for how arguments are formatted, those
 * functions let us do that on a case by case basis.
 *
 * Keys are lowercased here so they can be accessed programatically using input
 * string segments that are converted to lowercase for ease of comparison.
 *
...
export const AIRCRAFT_COMMAND_MAP = {
  ...
  airac : {aliases : [ 'airac' ], functionName : '', isSystemCommand : true},
  airport :
      {aliases : [ 'airport' ], functionName : '', isSystemCommand : true},
  altitude : {
    aliases : [ 'a', 'altitude', 'c', 'cm', 'climb', 'd', 'dm', 'descend' ],
    functionName : 'runAltitude',
    isSystemCommand : false
  },
  auto : {aliases : [ 'auto' ], functionName : '', isSystemCommand : true},
  clear : {aliases : [ 'clear' ], functionName : '', isSystemCommand : true},
  clearedAsFiled : {
  ...
  hold :
      {aliases : [ 'hold' ], functionName : 'runHold', isSystemCommand : false},
  ils : {
    aliases : [ '*', 'i', 'ils' ],
    functionName : 'runIls',
    isSystemCommand : false
  ...
  land :
      {aliases : [ 'land' ], functionName : 'runLand', isSystemCommand : false},
  moveDataBlock : {
    aliases : [ '`' ],
    functionName : 'runMoveDataBlock',
    isSystemCommand : false
  },
  pause : {aliases : [ 'pause' ], functionName : '', isSystemCommand : true},
  rate : {aliases : [ 'rate' ], functionName : '', isSystemCommand : true},
  reroute : {
    aliases : [ 'reroute', 'rr' ],
  ...
  sid : {aliases : [ 'sid' ], functionName : 'runSID', isSystemCommand : false},
  speed : {
  ...
  star :
      {aliases : [ 'star' ], functionName : 'runSTAR', isSystemCommand : false},
  takeoff : {
  ...
  tutorial :
      {aliases : [ 'tutorial' ], functionName : '', isSystemCommand : true}
};
...
export function findCommandNameWithAlias(commandAlias) {
  return _findKey(AIRCRAFT_COMMAND_MAP,
