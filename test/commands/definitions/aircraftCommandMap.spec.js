...
import {
  AIRCRAFT_COMMAND_MAP
    ...
    '../../../src/assets/scripts/client/commands/aircraftCommand/aircraftCommandMap';
import AircraftCommandModel from
    '../../../src/assets/scripts/client/commands/aircraftCommand/AircraftCommandModel';
import {
  altitudeParser,
  crossingParser,
  headingParser,
...
ava('aliases, noop parser and zeroArgumentsValidator used by clearedAsFiled',
    t => {
      const [parse, validate] = extractParseAndValidate(t, 'clearedAsFiled');
      t.true(parse === noopParse() && validate === zeroArgVal());
      test_aliases(t, AIRCRAFT_COMMAND_MAP, 'clearedAsFiled',
                   [ 'caf', 'clearedAsFiled' ]);
    });
...
ava('aliases, noop parser and zeroArgumentsValidator used by flyPresentHeading',
    t => {
...
ava('aliases, noop parser and zeroArgumentsValidator used by takeoff', t => {
  ...
  t.true(parse === noopParse() && validate === zeroArgVal());
  test_aliases(t, AIRCRAFT_COMMAND_MAP, 'takeoff',
               [ '/', 'cto', 'to', 'takeoff' ]);
});

ava('aliases, noop parser and zeroArgumentsValidator used by sayAltitude',
    t => {
...
ava('aliases, noop parser and zeroArgumentsValidator used by sayAssignedAltitude',
    t => {
      const [parse, validate] =
          extractParseAndValidate(t, 'sayAssignedAltitude');
      t.true(parse === noopParse() && validate === zeroArgVal());
...
ava('aliases, noop parser and zeroArgumentsValidator used by sayAssignedHeading',
    t => {
      const [parse, validate] =
          extractParseAndValidate(t, 'sayAssignedHeading');
      t.true(parse === noopParse() && validate === zeroArgVal());
      test_aliases(t, AIRCRAFT_COMMAND_MAP, 'sayAssignedHeading', [ 'sah' ]);
    });

ava('aliases, noop parser and zeroArgumentsValidator used by sayIndicatedAirspeed',
    t => {
      const [parse, validate] =
          extractParseAndValidate(t, 'sayIndicatedAirspeed');
      t.true(parse === noopParse() && validate === zeroArgVal());
      test_aliases(t, AIRCRAFT_COMMAND_MAP, 'sayIndicatedAirspeed', [ 'si' ]);
    });

ava('aliases, noop parser and zeroArgumentsValidator used by sayAssignedSpeed',
    t => {
...
ava('aliases, noop parser and singleArgumentValidator used by expectArrivalRunway',
    t => {
      const [parse, validate] =
          extractParseAndValidate(t, 'expectArrivalRunway');
      t.true(parse === noopParse() && validate === singleArgVal());
...
ava('aliases, noop parser and singleArgumentValidator used by moveDataBlock',
    t => {
...
ava('aliases, strToNumArray parser and singleArgumentValidator used by speed',
    t => {
      const [parse, validate] = extractParseAndValidate(t, 'speed');
      t.true(parse === strToNumArrayParse() && validate === singleArgVal());
      test_aliases(t, AIRCRAFT_COMMAND_MAP, 'speed',
                   [ '-', '+', 'slow', 'sp', 'speed' ]);
    });
...
ava('aliases, noop parser and zeroOrOneArgumentValidator used by cancelHold',
    t => {
      const [parse, validate] = extractParseAndValidate(t, 'cancelHold');
      t.true(parse === noopParse() && validate === zeroOrOneArgumentVal());
      test_aliases(t, AIRCRAFT_COMMAND_MAP, 'cancelHold',
                   [ 'exithold', 'cancelhold', 'continue', 'nohold', 'xh' ]);
    });
...
ava('aliases, altitude parser and altitude validator used by altitude', t => {
  ...
  test_aliases(t, AIRCRAFT_COMMAND_MAP, 'altitude',
               [ 'a', 'altitude', 'c', 'cm', 'climb', 'd', 'dm', 'descend' ]);
});
...
ava('aliases, heading parser and heading validator used by heading', t => {
  ...
  t.true(parse === p.toString() && validate === v.toString());
  test_aliases(t, AIRCRAFT_COMMAND_MAP, 'heading',
               [ 'fh', 'h', 'heading', 't', 'tl', 'tr', 'turn' ]);
});
...
ava('aliases, optionalAltitudeParser and optionalAltitudeValidator used by descendViaStar',
    t => {
      ...
      t.true(parse === p.toString() && validate === v.toString());
      test_aliases(t, AIRCRAFT_COMMAND_MAP, 'descendViaStar',
                   [ 'descendViaStar', 'dvs' ]);
    });

ava('aliases, optionalAltitudeParser and optionalAltitudeValidator used by climbViaSid',
    t => {
      const [parse, validate] = extractParseAndValidate(t, 'climbViaSid');
      const p = optionalAltitudeParser;
      const v = optionalAltitudeValidator;
      t.true(parse === p.toString() && validate === v.toString());
      test_aliases(t, AIRCRAFT_COMMAND_MAP, 'climbViaSid',
                   [ 'climbViaSid', 'cvs' ]);
    });

ava('make sure we test all 32 aircraft commands', t => {
  t.true(Object.values(AIRCRAFT_COMMAND_MAP)
             .filter(val => !val.isSystemCommand)
             .length === 32);
});
